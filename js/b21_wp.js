class B21_WP {
    ...
    new_point(index, position) {
        ...
        this.alt_m = 0;
        this.alt_m_updated = false; // true if elevation has been updated
        this.radius_m = null;
        this.max_alt_m = null;
        this.min_alt_m = null;
    ...
    wp_click() {
        this.task.select_waypoint(this.index);
    }

    get_icon() {
        let icon_str = (this.get_name()).replaceAll(" ", "&nbsp;");
        let class_name = (this.task.index == this.index) ? "wp_icon_html_current" : "wp_icon_html";
    ...
    getSecondLine() {
        ...
        if (this.index === 0 || this.index === this.task.waypoints.length - 1) {
            ...
            if (this.leg_distance_m) {
                secondLine += ` Dist: ${this.planner.tb.userSettings.distance === 'metric'
                    ? (this.leg_distance_m / 1000).toFixed(1) + ' km'
                    : (this.leg_distance_m * 0.000621371).toFixed(1) + ' mi'}`;
            }
        } else {
            if (this.isAAT()) {
                secondLine += 'AAT - ';
            ...
            if (this.radius_m) {
                if (this.planner.tb.userSettings.distance === 'metric') {
                    secondLine += this.radius_m >= 5000
                        ? `Radius: ${(this.radius_m / 1000).toFixed(1)} km `
                        : `Radius: ${Math.round(this.radius_m)} m `;
                } else {
                    const radiusFeet = this.radius_m * 3.28084;
                    secondLine += radiusFeet >= 5280
                        ? `Radius: ${(radiusFeet / 5280).toFixed(1)} mi `
                        : `Radius: ${Math.round(radiusFeet)}' `;
                }
            }
            if (this.min_alt_m) {
                secondLine += this.planner.tb.userSettings.altitude === 'imperial'
                    ? `MIN: ${Math.round(this.min_alt_m * 3.28084)}' `
                    : `MIN: ${Math.round(this.min_alt_m)} m `;
            }
            if (this.max_alt_m) {
                secondLine += this.planner.tb.userSettings.altitude === 'imperial'
                    ? `MAX: ${Math.round(this.max_alt_m * 3.28084)}' `
                    : `MAX: ${Math.round(this.max_alt_m)} m `;
            }
            if (this.leg_distance_m) {
                secondLine += this.planner.tb.userSettings.distance === 'metric'
                    ? ` Dist: ${(this.leg_distance_m / 1000).toFixed(1)} km`
                    : ` Dist: ${(this.leg_distance_m * 0.000621371).toFixed(1)} mi`;
            }
        }
        return secondLine;
    }
